...
const ApiToolsLayout = ({ children }: { children: React.ReactNode }) => {
    const [apiTools, setApiTools] = useState<ToolCategory | undefined>(undefined);

    useEffect(() => {
        const loadedTools = tools.find(tool => tool.name === 'API Tools');
        setApiTools(loadedTools);
    }, []);
    ...
    return (
        <main className="flex flex-col items-center min-h-screen p-8 bg-gradient-to-b from-gray-50 to-gray-200">
            <header className="text-center mb-12">
                <h1 className="text-5xl font-bold text-gray-900 mb-4">API Tools</h1>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-4xl w-full px-4">
                {apiTools.subcategories.map((subcategory) => (
                    <div
